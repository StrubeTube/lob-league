...
/* the rivalry board (Trade Archive) */
.flowwrap{display:flex;flex-direction:column;gap:8px}
.rivrow{display:grid;grid-template-columns:minmax(105px,150px) 1fr minmax(105px,150px) 56px;gap:10px;align-items:center;min-height:32px}
.rivname{font-size:12.5px;color:var(--ink2);min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.rivname.ra{text-align:right}
.rivname.win b{color:var(--mint)}
.rivname.lose{opacity:.6}
.rivval{color:var(--mint);font-weight:700;font-size:11px;margin-left:5px;font-variant-numeric:tabular-nums}
.rivtrack{position:relative;height:26px;background:var(--card2);border:1px solid var(--line);border-radius:9px;overflow:hidden}
.rivmid{position:absolute;left:50%;top:0;bottom:0;width:0;border-left:2px dashed var(--line2)}
.rivbar{position:absolute;top:50%;transform:translateY(-50%);left:50%;background:var(--mint);border-radius:0 99px 99px 0}
.rivbar.left{left:auto;right:50%;border-radius:99px 0 0 99px}
.rivdot{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);background:var(--ink3);border-radius:50%;opacity:.55}
.rivdeals{font-size:11px;color:var(--ink3);white-space:nowrap;font-variant-numeric:tabular-nums}
.rivdeals b{color:var(--ink2);font-size:12.5px}
...
.rivrow{grid-template-columns:1fr auto 1fr;grid-template-areas:'na nd nb' 'tr tr tr';row-gap:5px;column-gap:8px;min-height:0;padding:2px 0}
.rivname{font-size:11.5px}
.rivval{font-size:10px;margin-left:4px}
.rivname.ra{grid-area:na;text-align:left}
.rivname.rb{grid-area:nb;text-align:right}
.rivtrack{grid-area:tr;height:22px}
.rivdeals{grid-area:nd}
